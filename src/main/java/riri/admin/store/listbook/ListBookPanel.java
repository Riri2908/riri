...
public class ListBookPanel extends JPanel {
    ...
    public ListBookPanel() {
        ...
        tablePanel.getTable().addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                int viewRow = tablePanel.getTable().rowAtPoint(e.getPoint());
                int col     = tablePanel.getTable().columnAtPoint(e.getPoint());
                if (col != 8 || viewRow < 0) return;

                // Chuyển từ view row → model row (đúng khi đang sort/filter)
                int modelRow = tablePanel.getTable().convertRowIndexToModel(viewRow);

                int colStart = tablePanel.getTable().getCellRect(viewRow, 8, true).x;
                int relX = e.getX() - colStart;

                Book[] bookArray = books.values().toArray(new Book[0]);
                Book book = bookArray[modelRow];

                if (relX < 55) {
                    new EditBook(book, () -> updateData());
                } else {
                    new DeleteBook(book, () -> updateData());
                }
            }
        });
    ...
    private void loadData() {
        for (Book b : books.values()) {
            ...
            // THÊM ĐOẠN NÀY ĐỂ LẤY TÊN THỂ LOẠI
            String categoryName = b.getCategory();

            tablePanel.addRow(new Object[]{
                    b.getName(),
                    b.getAuthor()    != null ? b.getAuthor()    : "",
                    b.getPublisher() != null ? b.getPublisher() : "",
                    categoryName, // SỬA CHỖ NÀY: Dùng biến categoryName
                    areaName,
                    shelfName,
